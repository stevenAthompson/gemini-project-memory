...
export class ProjectDatabase {
  ...
  private registerFunctions() {
    this.db.function('regexp', (pattern: string, text: string) => {
      try {
   ...
   */
  public managePhase(action: 'init' | 'update', phaseId: string, title?: string, status?: string, description?: string): string {
    if (action === 'init') {
      if (!title) throw new Error('Title is required for initialization.');
      ...
      if (info.changes === 0) throw new Error(`Phase ${phaseId} not found.`);
   ...
   */
  public logWork(phaseId: string, action: string, outcome: string, artifacts: string[] = []): string {
   ...
   */
  public addPhaseItem(phaseId: string, type: 'objective' | 'finding' | 'next_step', content: string): string {
    let table = '';
    if (type === 'objective') table = 'objectives';
    else if (type === 'finding') table = 'findings';
    else if (type === 'next_step') table = 'next_steps';
   ...
   */
  public addUsageExample(phaseId: string, title: string, code: string, description: string = ''): string {
   ...
   * Adds a plan step.
   */
  public managePlan(phaseId: string, step: string): string {
   ...
   */
  public recordLesson(phaseId: string, text: string): string {
   ...
   */
  public manageInventory(filePath: string, description: string, category: string = 'Uncategorized', status: string = 'Active', usage: string | null = null): string {
   ...
   */
  public manageOverview(mission?: string, architecture?: string, faqQuestion?: string, faqAnswer?: string, phaseId?: string): string {
    if (mission) this.db.prepare('INSERT OR REPLACE INTO project_info (key, value) VALUES (?, ?)').run('mission', mission);
    if (architecture) this.db.prepare('INSERT OR REPLACE INTO project_info (key, value) VALUES (?, ?)').run('architecture', architecture);
   ...
   */
  public performSearch(query: string, type: 'keyword' | 'regex', tables?: string[]): string[] {
    const searchAll = !tables || tables.length === 0;
    const results: string[] = [];
    const operator = type === 'regex' ? 'REGEXP' : 'LIKE';
    const param = type === 'regex' ? query : `%${query}%`;

    const runQuery = (sql: string, params: any[]) => this.db.prepare(sql).all(...params) as any[];
  ...
  public ensureDirs() {
    ...
    if (!fs.existsSync(PHASES_DIR)) fs.mkdirSync(PHASES_DIR);
   ...
   */
  public generateReport(type: string, pid?: string, searchQuery?: string, searchType?: 'keyword' | 'regex'): string {
    this.ensureDirs();
    let output = '';
    let filename = '';

    if (type === 'phase') {
      if (!pid) throw new Error('phase_id required for phase report');
      
      const phase = this.db.prepare('SELECT * FROM phases WHERE id = ?').get(pid) as any;
      if (!phase) throw new Error(`Phase ${pid} not found`);
      
      const objs = this.db.prepare('SELECT text FROM objectives WHERE phase_id = ?').all(pid) as any[];
      const logs = this.db.prepare('SELECT * FROM execution_logs WHERE phase_id = ?').all(pid) as any[];
      const findings = this.db.prepare('SELECT text FROM findings WHERE phase_id = ?').all(pid) as any[];
      const nexts = this.db.prepare('SELECT text FROM next_steps WHERE phase_id = ?').all(pid) as any[];
      const usages = this.db.prepare('SELECT * FROM usage_examples WHERE phase_id = ?').all(pid) as any[];
      const planItems = this.db.prepare('SELECT * FROM plans WHERE phase_id = ? ORDER BY id').all(pid) as any[];
      ...
      const items = this.db.prepare('SELECT * FROM inventory ORDER BY category, path').all() as any[];
      const lines = [
        '# Project Inventory',
        '',
      ...
      const lessons = this.db.prepare('SELECT l.*, p.title as phase_title FROM lessons l JOIN phases p ON l.phase_id = p.id ORDER BY l.timestamp').all() as any[];
      const lines = [
        '# Lessons Learned',
        '',
      ...
      const mission = this.db.prepare("SELECT value FROM project_info WHERE key = 'mission'").get() as any;
      const arch = this.db.prepare("SELECT value FROM project_info WHERE key = 'architecture'").get() as any;
      const phases = this.db.prepare('SELECT * FROM phases ORDER BY id').all() as any[];
      const faqs = this.db.prepare('SELECT * FROM faqs WHERE phase_id IS NULL').all() as any[];
      ...
      if (!searchQuery) throw new Error('search_query required for search report');
   ...
   */
  public generateReports(reportType: 'phase' | 'inventory' | 'lessons' | 'overview' | 'all' | 'search', phaseId?: string, searchQuery?: string, searchType?: 'keyword' | 'regex'): string {
    ...
    if (reportType === 'all') {
      ...
      const phases = this.db.prepare('SELECT id FROM phases').all() as any[];
